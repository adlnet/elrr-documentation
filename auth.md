# ELRR Token Authentication

API access to ELRR Services is protected by token-based authentication. To access the API, clients must obtain a valid access token from the Token Endpoint and include it in the Authorization header of their requests using the format `Bearer <token>`.

Access to the token endpoint (and any other administrative endpoints) is protected by the presence of a specific role in the payload of a JWT issued by your Identity Provider, the "Admin JWT". The Admin JWT is used to request API tokens which can then be used to access the rest of the API. In development, the Admin JWT can also be used directly to access the API if the `client.admin-api-override` property is set to true (see [Admin API Override Mode](#admin-api-override-mode) below).

## Token Endpoint

To get a token and make API calls you can POST a request to the `/admin/token` endpoint. Authentication for this endpoint is based on the presence of a given role in the payload of a JWT issued by your Identity Provider. This role is defined by the application's `admin.jwt.role` and `admin.jwt.role-key` settings, for instance:
